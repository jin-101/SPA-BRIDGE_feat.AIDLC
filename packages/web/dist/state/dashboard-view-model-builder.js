...
const toCount = (value) => String(value);
const makeSummaryCard = (id, title, rows) => createSectionModel(id, title, rows.map(([label, value]) => createKeyValueRow(label, value)));
const deriveQualityStatus = (report, summary) => {
...
export const buildDashboardViewModel = (report, qualitySummary, access, navigation, layout) => {
    ...
    return {
        ...
        subtitle: renderSafeText(`${summaryStatus} :: ${report.conversionOutput.convertedArtifactCount}/${report.conversionOutput.totalCandidateArtifactCount} converted`).text,
        summaryCards: [
...
//# sourceMappingURL=dashboard-view-model-builder.js.map