...
import type { WebAccessState, WebReviewRole, WebReviewTab } from '../types.js';
import { createWebUiError, type WebUiError } from '../shared-errors.js';

const tabsByRole: Record<WebReviewRole, WebReviewTab[]> = {
  guest: ['dashboard'],
...
const actionsByRole: Record<WebReviewRole, string[]> = {
  guest: [],
...
export const buildAccessGateState = (input: {
  ...
  const allowedTabs: WebReviewTab[] = decision.decision === 'allow' ? tabsByRole[input.role] : ['dashboard', 'reports'];
  const allowedActions: string[] = decision.decision === 'allow' ? actionsByRole[input.role] : ['open-report'];
