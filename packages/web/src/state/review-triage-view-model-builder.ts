...
import type { WebAccessState, WebSectionModel, WebReviewTriageViewModel } from '../types.js';
...
const compareReviewItems = (left: { severity: string; category?: string; id: string }, right: { severity: string; category?: string; id: string }): number =>
  (severityOrder[left.severity] ?? 99) - (severityOrder[right.severity] ?? 99) ||
  (left.category ?? '').localeCompare(right.category ?? '') ||
  left.id.localeCompare(right.id);

const sectionFromItems = (id: string, title: string, rows: Array<[string, string]>): WebSectionModel =>
  createSectionModel(id, title, rows.map(([label, value]) => createKeyValueRow(label, value)));

export const buildReviewTriageViewModel = (report: CanonicalConversionReport, access: WebAccessState): WebReviewTriageViewModel => {
  ...
  const reviewItems = sortedItems.map((item) =>
    createSectionModel(`review-item-${item.id}`, item.id, [
  ...
  const reviewGroups = sortedGroups.map((group) =>
    sectionFromItems(`review-group-${group.id}`, group.id, [
