...
import { createSecurityError, SecurityConfigInputSchema, SecurityConfigSchema, } from '../types.js';
const mergeRulePackIds = (defaults, raw, overrides) => [...defaults, ...raw, ...overrides].filter((value, index, values) => values.indexOf(value) === index);
export class SecurityConfigResolver {
    resolve(input) {
        ...
        const rawConfig = rawResult.value;
        ...
        const merged = SecurityConfigSchema.parse({
            ...
            allowExternalProviderUse: overrides.allowExternalProviderUse ?? rawConfig.allowExternalProviderUse ?? defaults.allowExternalProviderUse,
            ...
            preservePartialArtifacts: overrides.preservePartialArtifacts ?? rawConfig.preservePartialArtifacts ?? defaults.preservePartialArtifacts,
            tokenTtlMs: overrides.tokenTtlMs ?? rawConfig.tokenTtlMs ?? defaults.tokenTtlMs,
            rulePackIds: mergeRulePackIds(defaults.rulePackIds, rawConfig.rulePackIds ?? [], overrides.rulePackIds ?? []),
            defaultProviderMode: overrides.defaultProviderMode ?? rawConfig.defaultProviderMode ?? defaults.defaultProviderMode,
            targetAwareRulePacks: overrides.targetAwareRulePacks ?? rawConfig.targetAwareRulePacks ?? defaults.targetAwareRulePacks,
        });
        return ok(merged);
    }
}
