...
export const buildReportViewModel = (report) => ({
    ...
    sections: {
        ...
        diagnostics: report.diagnostics.map((diagnostic) => row(diagnostic.id, [diagnostic.severity, diagnostic.reasonCode, diagnostic.safeMessage, refText(diagnostic.sourceRef), refText(diagnostic.generatedRef)]
            .filter((value) => value && value !== 'N/A')
            .join(' | '))),
        aiDecisions: report.aiDecisions.map((decision) => row(decision.id, [
        ...
        manualReview: [
            ...report.manualReview.items.map((item) => row(item.id, [item.severity, item.category, item.reasonCode, item.safeSummary, refText(item.sourceRef), refText(item.generatedRef)]
                .filter((value) => value && value !== 'N/A')
                .join(' | '))),
            ...report.manualReview.groups.map((group) => row(group.id, [group.severity, group.reviewCategory ?? 'manual-review', group.itemIds.join(', '), refText(group.sourceRef), refText(group.generatedRef)]
                .filter((value) => value && value !== 'N/A')
                .join(' | '))),
        ],
        quality: [
