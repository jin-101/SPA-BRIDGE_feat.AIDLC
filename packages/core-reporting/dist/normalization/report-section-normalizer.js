...
const sourceRefSortKey = (ref) => ref ? `${ref.path}::${ref.symbol ?? ''}::${ref.location ?? ''}` : '';
const generatedRefSortKey = (ref) => ref ? `${ref.path}::${ref.segment ?? ''}` : '';
const severityOrder = {
    blocking: 0,
    critical: 1,
    warning: 2,
...
const sortByStableKey = (items, keyFn) => [...items].sort((left, right) => compareStrings(keyFn(left), keyFn(right)));
...
const sortRecordKeys = (record) => Object.fromEntries(Object.entries(record).sort(([left], [right]) => compareStrings(left, right)));
export const normalizeDiagnostics = (diagnostics) => sortByStableKey(uniqueBy(diagnostics, (item) => item.id), (item) => [
...
export const normalizeManualReviewItems = (items) => sortByStableKey(uniqueBy(items, (item) => item.id), (item) => [
    severityOrder[item.severity] ?? 99,
    item.category,
    sourceRefSortKey(item.sourceRef),
    generatedRefSortKey(item.generatedRef),
...
export const normalizeAiDecisions = (items) => sortByStableKey(uniqueBy(items, (item) => item.id), (item) => item.id);
export const normalizeTraceLinks = (links) => sortByStableKey(uniqueBy(links, (item) => item.id), (item) => [item.id, item.source.path, generatedRefSortKey(item.target.kind === 'generated' ? item.target : undefined), item.relation].join('|'));
export const normalizeQualitySection = (quality, targetPercent) => ({
    ...quality,
...
export const normalizeReportInputBundle = (bundle, targetPercent = 85) => ({
    ...
    sourceInventory: {
        ...
        sourceRefs: sortByStableKey(uniqueBy(bundle.sourceInventory.sourceRefs, (item) => `${item.kind}:${item.path}:${item.symbol ?? ''}:${item.location ?? ''}`), (item) => [item.path, item.symbol ?? '', item.location ?? ''].join('|')),
    ...
    conversionOutput: {
        ...
        generatedRefs: sortByStableKey(uniqueBy(bundle.conversionOutput.generatedRefs, (item) => `${item.kind}:${item.path}:${item.segment ?? ''}`), (item) => [item.path, item.segment ?? ''].join('|')),
    },
    ...
    traceability: {
        ...
        syntheticOrigins: sortByStableKey(uniqueBy(bundle.traceability.syntheticOrigins, (item) => `${item.kind}:${item.path}:${item.segment ?? ''}`), (item) => [item.path, item.segment ?? ''].join('|')),
        coverageSummary: {
...
//# sourceMappingURL=report-section-normalizer.js.map