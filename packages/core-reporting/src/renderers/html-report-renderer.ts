...
const renderList = (rows: Record<string, string>[]): string => {
  if (rows.length === 0) {
    return '<p>None</p>';
  }
...
const renderSection = (title: string, rows: Record<string, string>[]): string =>
  `<section><h2>${escapeHtml(title)}</h2>${renderList(rows)}</section>`;

export const renderHtmlReport = (report: CanonicalConversionReport): string => {
