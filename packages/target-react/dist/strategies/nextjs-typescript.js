...
const buildNextConfig = (aliasMappings) => {
    const aliasLines = aliasMappings.map((mapping) => `      config.resolve.alias['${mapping.alias}'] = path.resolve(process.cwd(), '${mapping.nextAliasTarget}');`);
...
const buildNextEnv = () => `/// <reference types="next" />
/// <reference types="next/image-types/global" />

// This file is generated by SPA-Bridge for the Next.js target.
`;
const buildGlobalStyles = () => `.app-shell {
  font-family: Inter, system-ui, sans-serif;
  padding: 2rem;
}
`;
const buildRootProvider = (includeReduxStore) => `'use client';
...
`;
const buildLayout = (projectName) => `import type { Metadata } from 'next';
...
`;
const buildPage = (projectName) => `export default function Page() {
  return (
...
`;
const buildStrategy = (defaultStrategy) => ({
...
export const createNextJsTypeScriptStrategy = () => buildStrategy(true);
export const nextJsTypeScriptStrategyId = 'nextjs-typescript';
export const createNextJsDependencyManifest = () => ({
...
//# sourceMappingURL=nextjs-typescript.js.map