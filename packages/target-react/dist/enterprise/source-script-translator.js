...
const classifyIntent = (name, command) => {
    const normalized = `${name} ${command}`.toLowerCase();
    if (UNSAFE_SCRIPT_PATTERN.test(command))
        return 'unsafe';
    if (/\bng\s+serve\b/.test(normalized))
        return 'dev';
    if (/\bng\s+build\b/.test(normalized))
        return 'build';
    if (/\bng\s+test\b|\bkarma\b/.test(normalized))
        return 'test';
    if (/\bng\s+lint\b|\beslint\b/.test(normalized))
        return 'lint';
    if (/\btsc\b/.test(normalized))
        return 'typecheck';
    if (/\bwebpack-bundle-analyzer\b|\banaly[sz]e\b/.test(normalized))
        return 'analyze';
    if (/\bdeploy\b|\bpublish\b|\bupload\b/.test(normalized))
        return 'deploy';
    if (/\bng\b|\bangular\b/.test(normalized))
        return 'angular-only';
    if (name === 'start')
        return 'start';
    return 'unknown';
};
const targetFor = (name, intent) => {
    switch (intent) {
        case 'dev':
        case 'build':
        case 'start':
...
export class SourceScriptTranslator {
    translate(sourceScripts = {}) {
        const decisions = Object.entries(sourceScripts)
            .map(([sourceName, sourceCommand]) => {
...
//# sourceMappingURL=source-script-translator.js.map