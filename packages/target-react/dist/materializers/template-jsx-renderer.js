...
const splitPipes = (expression) => {
    const [base = expression, ...pipes] = expression.split('|').map((part) => part.trim()).filter(Boolean);
    return { base, pipes };
};
const renderPipeExpression = (expression, context, helpers, diagnostics) => {
...
const transformNgClass = (expression, context, helpers) => {
    helpers.add('applyClassMap');
    return `className={applyClassMap(${context.transformExpression(expression)})}`;
};
const transformNgStyle = (expression, context, helpers) => {
...
export class TemplateJsxRenderer {
    render(templateText, templateIr, context, selectorRegistry) {
        if (!templateText?.trim()) {
            return { lines: [], usedSelectors: [], helpers: [], diagnostics: [] };
        }
        const usedSelectors = new Set();
        const helpers = new Set();
        const diagnostics = [...(templateIr?.diagnostics.map((diagnostic) => diagnostic.code) ?? [])];
        const deferredTemplates = new Map();
        let jsx = templateText.slice(0, 12_000).replace(/<!--[\s\S]*?-->/g, '');
        jsx = jsx.replace(/<ng-template\s+#([A-Za-z_$][\w$]*)[^>]*>([\s\S]*?)<\/ng-template>/g, (_match, name, body) => {
        ...
        jsx = jsx.replace(/<([A-Za-z][A-Za-z0-9-]*)([^>]*)\s\*ngIf="([^"]+)"([^>]*)>([\s\S]*?)<\/\1>/g, (_match, tag, before, expression, after, body) => {
        ...
        jsx = jsx.replace(/<([A-Za-z][A-Za-z0-9-]*)([^>]*)\s\*ngFor="([^"]+)"([^>]*)>([\s\S]*?)<\/\1>/g, (_match, tag, before, expression, after, body) => {
        ...
        jsx = jsx
            ...
            .replace(/\{\{\s*([^}]+?)\s*\}\}/g, (_match, expression) => `{${renderPipeExpression(expression, context, helpers, diagnostics)}}`)
            .replace(/\s(src|href)=["']([^"']+)["']/g, (_match, attr, value) => {
...
//# sourceMappingURL=template-jsx-renderer.js.map