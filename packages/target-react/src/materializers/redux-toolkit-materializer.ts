...
import type { ReactReduxToolkitDraft } from '@spa-bridge/transform-angular-react';
...
const toFeatureSegment = (value: string): string => toIdentifier(value, 'feature').replace(/_/g, '-').toLowerCase();

const asComment = (value: string): string => value.replace(/\*\//g, '* /');

const collectReducerActionNames = (draft: ReactReduxToolkitDraft): string[] => {
...
const renderSlice = (draft: ReactReduxToolkitDraft): GeneratedFileSpec => {
...
const renderSelectors = (draft: ReactReduxToolkitDraft): GeneratedFileSpec => {
  const feature = toIdentifier(draft.featureName, 'feature');
...
const renderEffects = (draft: ReactReduxToolkitDraft): GeneratedFileSpec | undefined => {
...
export class ReduxToolkitMaterializer {
  materialize(drafts: ReactReduxToolkitDraft[], sourceRefs: SourceRef[] = []): GeneratedFileSpec[] {
    if (drafts.length === 0) {
      return [];
    }
    ...
    return [
      indexFile,
      hooksFile,
      ...sortedDrafts.flatMap((draft) => [renderSlice(draft), renderSelectors(draft), renderEffects(draft)].filter((file): file is GeneratedFileSpec => Boolean(file))),
    ];
  }
}
