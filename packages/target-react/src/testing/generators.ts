...
const createDraftSetArbitrary = (): fc.Arbitrary<ReactTargetDraftSet> =>
  fc
    .record({
      schemaVersion: fc.constant<1>(1),
      targetFramework: fc.constant<'react'>('react'),
      ...
      services: fc.array(
        fc.record({
          ...
          name: identifierArbitrary,
          sourceRef: fc.option(sourceRefArbitrary, { nil: undefined }),
          kind: fc.constantFrom<'module' | 'hook' | 'context' | 'unknown'>('module', 'hook', 'context', 'unknown'),
      ...
      state: fc.array(
        fc.record({
          ...
          name: identifierArbitrary,
          sourceRef: fc.option(sourceRefArbitrary, { nil: undefined }),
          strategy: fc.constantFrom<TargetStateStrategy>('service', 'signals', 'store', 'local', 'unknown'),
      ...
      traces: fc.array(
        fc.record({
          ...
          relation: fc.constantFrom<'maps-to' | 'derived-from' | 'emits' | 'references'>('maps-to', 'derived-from', 'emits', 'references'),
          confidence: fc.double({ min: 0, max: 1, noNaN: true, noDefaultInfinity: true }),
        }),
        { maxLength: 6 },
      ),
    })
    .map((draftSet): ReactTargetDraftSet => ({
      ...draftSet,
      projectStrategy: draftSet.projectStrategy as ReactTargetDraftSet['projectStrategy'],
      ...
      diagnostics: [...draftSet.diagnostics].sort((left, right) => left.code.localeCompare(right.code)) as ReactTargetDraftSet['diagnostics'],
      traces: [...draftSet.traces].sort((left, right) => left.id.localeCompare(right.id)) as ReactTargetDraftSet['traces'],
    }));

export const targetGenerationRequestArbitrary: fc.Arbitrary<TargetGenerationRequest> = fc
  .record({
    runId: identifierArbitrary,
    correlationId: identifierArbitrary,
    targetRoot: absolutePathArbitrary,
    strategyId: fc.option(fc.constantFrom<TargetProjectStrategyId>('nextjs-typescript', 'vite-react-typescript', 'react-default'), { nil: undefined }),
  ...
  .chain((fields) =>
    createDraftSetArbitrary().map((draftSet): TargetGenerationRequest => ({
      runId: fields.runId,
