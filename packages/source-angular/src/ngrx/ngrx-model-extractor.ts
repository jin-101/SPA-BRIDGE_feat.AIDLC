...
const toSourceRef = (path: string, symbol?: string): SourceRef => ({
  kind: 'source',
  path,
  symbol,
});
...
const toFeatureName = (name: string): string =>
  name
    .replace(/Reducer$/i, '')
    .replace(/Selectors?$/i, '')
    .replace(/^select/i, '')
...
const getBalancedCall = (sourceText: string, callName: string, startIndex: number): string => {
...
const splitTopLevelArgs = (text: string): string[] => {
...
const extractPayloadProperties = (expression: string): string[] => {
...
const detectOperators = (expression: string): string[] => {
  const operatorNames = [
    'map',
    'switchMap',
    'mergeMap',
...
const getSourceText = (sourcePath: string): string => {
  try {
...
export class NgrxModelExtractor {
  ...
  extract(typeScriptSummaries: TypeScriptParseSummary[]): AngularNgrxModel {
    const actions: AngularNgrxActionModel[] = [];
    const reducers: AngularNgrxReducerModel[] = [];
    const selectors: AngularNgrxSelectorModel[] = [];
    const effects: AngularNgrxEffectModel[] = [];
    const entityAdapters: AngularNgrxEntityAdapterModel[] = [];
    const componentUsages: AngularNgrxComponentUsageModel[] = [];
  ...
  private extractActions(sourcePath: string, sourceText: string): AngularNgrxActionModel[] {
    const actions: AngularNgrxActionModel[] = [];
  ...
  private extractReducers(sourcePath: string, sourceText: string): AngularNgrxReducerModel[] {
    const reducers: AngularNgrxReducerModel[] = [];
    ...
    for (const match of sourceText.matchAll(reducerRegex)) {
      ...
      const handlers: AngularNgrxReducerHandlerModel[] = [];
  ...
  private extractSelectors(sourcePath: string, sourceText: string): AngularNgrxSelectorModel[] {
    const selectors: AngularNgrxSelectorModel[] = [];
  ...
  private extractEffects(sourcePath: string, sourceText: string): AngularNgrxEffectModel[] {
    const effects: AngularNgrxEffectModel[] = [];
  ...
  private extractEntityAdapters(sourcePath: string, sourceText: string): AngularNgrxEntityAdapterModel[] {
    const adapters: AngularNgrxEntityAdapterModel[] = [];
  ...
  private extractComponentUsages(summary: TypeScriptParseSummary, sourceText: string): AngularNgrxComponentUsageModel[] {
    const usages: AngularNgrxComponentUsageModel[] = [];
