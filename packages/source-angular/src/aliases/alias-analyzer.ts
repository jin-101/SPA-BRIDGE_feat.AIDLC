  ...
  projects?: Record<
    string,
    {
      ...
      architect?: {
        build?: {
          options?: {
            assets?: Array<string | { input?: string }>;
          };
        };
      };
    }
...
const aliasId = (prefix: string, value: string): string =>
  `${prefix}-${value.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'root'}`;

const normalizeAliasTarget = (target: string): string => target.replace(/\*+$/g, '').replace(/\/+$/g, '');

const statusPriority: Record<AliasResolutionStatus, number> = {
  supported: 0,
  unresolved: 1,
  external: 2,
  unsafe: 3,
...
export class AliasAnalyzer {
  ...
  async analyze(projectRoot: string, sourceRoot: string): Promise<SourceAliasModel> {
  ...
  private async readTsConfigChain(projectRoot: string, diagnostics: AliasDiagnostic[]): Promise<TsConfigRecord[]> {
  ...
  private async readTsConfigRecursive(
    projectRoot: string,
    filePath: string,
    ...
    const config = parsed.config as TsConfigJson;
  ...
  private resolveBaseUrl(projectRoot: string, configs: TsConfigRecord[]): string | undefined {
  ...
  private buildPathAliases(projectRoot: string, baseUrl: string, configs: TsConfigRecord[], diagnostics: AliasDiagnostic[]): PathAliasMapping[] {
    const mappings = new Map<string, PathAliasMapping>();
    for (const record of configs) {
      ...
      for (const [aliasPattern, targets] of Object.entries(paths)) {
        ...
        let status: AliasResolutionStatus = 'supported';
  ...
  private async readAngularJson(filePath: string, diagnostics: AliasDiagnostic[]): Promise<AngularJson | undefined> {
    try {
      const text = await fs.readFile(filePath, 'utf8');
      return JSON.parse(text) as AngularJson;
    } catch {
  ...
  private buildWorkspaceProjects(projectRoot: string, angularJson: AngularJson | undefined, diagnostics: AliasDiagnostic[]): AngularWorkspaceProjectAlias[] {
    ...
    return entries.map(([projectName, project]) => {
      ...
      const status: AliasResolutionStatus = !projectContained.ok || (sourceContained && !sourceContained.ok) ? 'unsafe' : 'supported';
      if (status === 'unsafe') {
  ...
  private buildAssetRoots(projectRoot: string, sourceRoot: string, angularJson: AngularJson | undefined): string[] {
  ...
  private diagnosticsForMapping(mapping: PathAliasMapping): AliasDiagnostic[] {
  ...
  private toAliasDiagnostic(
    code: string,
    severity: AliasDiagnostic['severity'],
    message: string,
    sourcePath?: string,
    aliasPattern?: string,
  ): AliasDiagnostic {
