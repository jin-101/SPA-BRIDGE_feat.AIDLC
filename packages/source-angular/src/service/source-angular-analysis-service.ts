...
const isTypeScriptFile = (record: FileInventoryRecord): boolean => record.kind === 'typescript' && !record.relativePath.endsWith('.spec.ts');
const isTemplateFile = (record: FileInventoryRecord): boolean => record.kind === 'template';
const isStyleFile = (record: FileInventoryRecord): boolean => record.kind === 'style';

const toSourceRef = (filePath: string, symbol?: string) => ({
...
export class SourceAngularAnalysisService {
  ...
  private readonly diagnosticBuilder = new SafeDiagnosticBuilder();
  private readonly artifactMapper = new AnalysisArtifactMapper();

  async analyze(request: SourceAngularAnalysisRequest): Promise<Result<AngularAnalysisResult, AnalysisError>> {
    ...
    const sourceModelBoundary = AngularSourceModelBoundarySchema.parse(
      this.artifactMapper.buildSourceModelBoundary(workspaceProfile, workspaceProfile.entryFiles[0] ?? path.join(workspaceProfile.sourceRoot, 'main.ts')),
    );
