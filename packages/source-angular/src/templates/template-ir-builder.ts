...
const stableId = (sourcePath: string, ordinal: number): string =>
  `tpl-${sourcePath.replace(/[^A-Za-z0-9]+/g, '-').replace(/^-|-$/g, '').toLowerCase()}-${ordinal}`;

const parsePipeUsage = (expression: string): TemplatePipeUsage[] => {
...
const parseNgFor = (expression: string): Record<string, string> => {
...
export class TemplateIrBuilder {
  build(sourcePath: string, templateText: string): TemplateIr {
    const rootNodes: TemplateIrNode[] = [];
    const diagnostics: TemplateConversionDiagnostic[] = [];
    let ordinal = 1;
