...
import type { AnalysisError, AngularWorkspaceProfile, FileInventoryRecord, FileRole, FileKind, SourceInventory } from '../types.js';

const EXCLUDED_DIRS = new Set(['node_modules', 'dist', 'coverage', '.git', '.spa-bridge']);

const classifyFile = (filePath: string): { role: FileRole; kind: FileKind; evidence: string[] } => {
...
const walk = async (root: string, current: string, collected: string[]): Promise<void> => {
...
export class SourceInventoryBuilder {
  ...
  async build(profile: AngularWorkspaceProfile): Promise<Result<SourceInventory, AnalysisError>> {
    const files: string[] = [];
    try {
    ...
    const inventory: FileInventoryRecord[] = [];
