...
const toValueName = (memberName: string): string => {
...
const extractCallArguments = (text: string, callName: string): string | undefined => {
...
const operatorKind = (name: string): AngularRxOperatorKind => {
  if (name === 'map') return 'projection';
  if (name === 'filter' || name === 'distinctUntilChanged') return 'filter';
  if (name === 'tap') return 'side-effect';
  if (name === 'switchMap' || name === 'mergeMap' || name === 'concatMap' || name === 'exhaustMap') return 'flattening';
  if (name === 'catchError' || name === 'retry') return 'error-handling';
  if (name === 'debounceTime' || name === 'delay' || name === 'throttleTime') return 'timing';
  if (name === 'shareReplay' || name === 'share') return 'sharing';
  if (name === 'takeUntil' || name === 'take' || name === 'first') return 'cleanup';
  return 'unknown';
};

const isObservableLike = (name: string, typeText?: string, initializer?: string): boolean =>
  /\$$/.test(name) ||
  /Observable\s*</.test(typeText ?? '') ||
  /(of|from|combineLatest|merge|interval|timer)\s*\(/.test(initializer ?? '') ||
  /\.pipe\s*\(/.test(initializer ?? '');

const subjectKind = (typeText?: string, initializer?: string): AngularRxSubjectModel['subjectKind'] => {
  const source = `${typeText ?? ''} ${initializer ?? ''}`;
  if (/BehaviorSubject/.test(source)) return 'BehaviorSubject';
  if (/ReplaySubject/.test(source)) return 'ReplaySubject';
  if (/AsyncSubject/.test(source)) return 'AsyncSubject';
  if (/Subject/.test(source)) return 'Subject';
  return 'unknown';
};
...
export class RxjsModelExtractor {
  ...
  extract(typeScriptSummaries: TypeScriptParseSummary[], templateSummaries: TemplateParseSummary[]): AngularRxModel {
    const streams: AngularRxStreamModel[] = [];
    const subjects: AngularRxSubjectModel[] = [];
    const subscriptions: AngularRxSubscriptionModel[] = [];
    const operatorChains: AngularRxOperatorChain[] = [];
        ...
        for (const property of symbol.propertyInitializers) {
          ...
          if (isObservableLike(property.name, property.typeText, property.initializer)) {
            const chain = property.initializer?.includes('.pipe(')
              ...
              : undefined;
            if (chain) operatorChains.push(chain);
            streams.push({
  ...
  private extractOperatorChain(sourcePath: string, expression: string, index: number): AngularRxOperatorChain | undefined {
    const args = extractCallArguments(expression, 'pipe');
    if (!args) return undefined;
    const operators = splitTopLevel(args).map((operator, operatorIndex): AngularRxOperatorModel => {
  ...
  private extractAsyncPipeBindings(templates: TemplateParseSummary[], streams: AngularRxStreamModel[]): AngularAsyncPipeBinding[] {
    const bindings: AngularAsyncPipeBinding[] = [];
    for (const template of templates) {
      ...
      for (const match of matches) {
        ...
        const stream = streams.find((candidate) =>
          candidate.memberName === expressionText ||
  ...
  private extractSubjectInitialValue(initializer?: string): string | undefined {
    if (!initializer || !/BehaviorSubject/.test(initializer)) return undefined;
    return extractCallArguments(initializer, 'BehaviorSubject')?.split(',')[0]?.trim();
  }

  private findMethodRefs(sourcePath: string, methods: TypeScriptParseSummary['symbols'][number]['methods'], callText: string): SourceRef[] {
    return methods
  ...
  private inferAssignmentTarget(callbackText: string): string | undefined {
    return callbackText.match(/this\.([A-Za-z_$][\w$]*)\s*=/)?.[1];
  }

  private cleanupEvidence(methodText: string, methods: TypeScriptParseSummary['symbols'][number]['methods']): AngularRxSubscriptionModel['cleanupEvidence'] {
    if (/takeUntil\s*\(/.test(methodText)) return 'takeUntil';
    if (/\.add\s*\(/.test(methodText)) return 'subscription-add';
    if (methods.some((method) => method.name === 'ngOnDestroy' && /\.unsubscribe\s*\(/.test(method.bodyText))) return 'ngOnDestroy-unsubscribe';
    return 'none';
  }

  private sideEffectLevel(callbackText: string): AngularRxSubscriptionModel['sideEffectLevel'] {
    if (!callbackText.trim()) return 'none';
    if (/this\.[A-Za-z_$][\w$]*\s*=/.test(callbackText)) return 'state-assignment';
    if (/this\.[A-Za-z_$][\w$]*\s*\(/.test(callbackText)) return 'method-call';
    if (/\.navigate\s*\(|\.dispatch\s*\(|\.post\s*\(|\.put\s*\(|\.delete\s*\(/.test(callbackText)) return 'external-effect';
    return 'unknown';
  }
}
