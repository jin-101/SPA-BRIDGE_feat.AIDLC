...
import type { AnalysisError, TypeScriptParseSummary, TypeScriptSymbolSummary } from '../types.js';
...
const parseDecorator = (decorator: ts.Decorator, sourceFile: ts.SourceFile): Record<string, string | boolean | string[]> => {
  ...
  for (const prop of firstArg.properties) {
    ...
    if (ts.isStringLiteral(value) || ts.isNoSubstitutionTemplateLiteral(value)) {
      ...
      result[key] = value.elements.map((element) => element.getText()).filter((text): text is string => text.length > 0);
    } else {
...
export class TypeScriptParserAdapter {
  ...
  parse(sourcePath: string, sourceText: string): Result<TypeScriptParseSummary, AnalysisError> {
    ...
    const parseDiagnostics = ((sourceFile as ts.SourceFile & { parseDiagnostics?: readonly ts.Diagnostic[] }).parseDiagnostics ?? []) as readonly ts.Diagnostic[];
    const diagnostics: Diagnostic[] = parseDiagnostics.map((issue: ts.Diagnostic, index: number) =>
      createDiagnostic({
        code: 'TS-PARSE-001',
        severity: 'warning',
    ...
    const symbols: TypeScriptSymbolSummary[] = [];
    const visit = (node: ts.Node): void => {
      if (ts.isClassDeclaration(node) && node.name) {
        ...
        const lifecycle = node.members
          .filter((member): member is ts.MethodDeclaration => ts.isMethodDeclaration(member) && !!member.name && ts.isIdentifier(member.name))
