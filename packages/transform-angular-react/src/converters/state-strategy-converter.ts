...
import type { ReactReduxToolkitDraft } from '../types.js';

const featureFrom = (name: string): string =>
  name
    .replace(/Reducer$/i, '')
    .replace(/^select/i, '')
    .replace(/[^A-Za-z0-9]+/g, '')
...
export class StateStrategyConverter {
  ...
  convert(context: TransformationContext): RuleContribution {
    ...
    const reduxToolkitDrafts: ReactReduxToolkitDraft[] = [...featureNames].sort((left, right) => left.localeCompare(right)).map((featureName, index) => {
    ...
    const stateDrafts = context.states.map((state) => {
      ...
      return {
        ...
        strategy: context.ngrxReducers.length > 0 || context.ngrxComponentUsages.length > 0 ? 'store' as const : state.strategy,
