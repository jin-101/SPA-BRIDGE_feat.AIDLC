...
import type { TransformationContext, TransformationRule, RuleContribution } from '../types.js';

const supportedPackages: Array<{ name: string; category: 'route' | 'state' | 'di' | 'template' | 'unknown'; hint: string }> = [
...
export const createAngularEcosystemRules = (): TransformationRule[] => {
        ...
        for (const packageRef of context.packageRefs) {
          ...
          mappingRequests.push(
            mappingBuilder.build(
              match.category,
              [{ kind: 'source', path: context.sourceModelRef.projectPath }],
              [ids.artifactRef(`${context.projectName}/${packageRef.replace(/[@/]/g, '-')}.json`, packageRef)],
