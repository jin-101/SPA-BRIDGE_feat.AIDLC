...
export const benchmarkAnalysisOptionsArbitrary = fc.record({
    componentCount: fc.integer({ min: 1, max: 3 }),
    includeUnsupportedTemplate: fc.boolean(),
});
export const benchmarkAnalysisArbitrary = benchmarkAnalysisOptionsArbitrary.map((options) => createBenchmarkAngularAnalysisFixture({
    componentCount: options.componentCount,
    includeUnsupportedTemplate: options.includeUnsupportedTemplate,
}));
export const transformationRequestArbitrary = benchmarkAnalysisOptionsArbitrary.chain((analysisOptions) => fc
    .record({
    ...
    targetProjectStrategy: requestFields.targetProjectStrategy,
    stateStrategy: requestFields.stateStrategy,
