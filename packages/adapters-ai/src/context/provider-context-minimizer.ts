...
const createProviderError = (code: ProviderError['code'], message: string): ProviderError =>
...
const dedupeSafeRefs = (refs: Array<SourceRef | { kind: 'generated'; path: string; segment?: string }>): Array<SourceRef | { kind: 'generated'; path: string; segment?: string }> =>
  refs.filter((ref, index, array) => array.findIndex((candidate) => candidate.kind === ref.kind && candidate.path === ref.path && ('segment' in candidate ? candidate.segment : undefined) === ('segment' in ref ? ref.segment : undefined)) === index);

export const createMinimizedProviderContext = (
  input: ProviderNeutralRefinementRequest,
  policyEvidenceRef: string,
): Result<MinimizedProviderContext, ProviderError> => {
  ...
  const safeContext: ProviderSafeContext = {};
  ...
  return ok(
    MinimizedProviderContextSchema.parse({
