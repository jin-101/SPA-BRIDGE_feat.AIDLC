...
import { ProviderDescriptorSchema, ProviderErrorSchema, ProviderSelectionRequestSchema, ProviderSelectionResultSchema, } from '../types.js';
...
const modeRank = (mode, kind) => {
    if (mode === 'external-only') {
        if (kind === 'external')
            return 0;
        if (kind === 'local-internal')
            return 1;
        return 2;
    }
    if (kind === 'local-internal')
...
const capabilityMatchScore = (provider, request) => {
...
const readinessScore = (provider, request) => {
...
const selectionComparator = (request) => (left, right) => {
    ...
    if (providerIdDelta !== 0)
        return providerIdDelta;
    return compareStrings(left.adapterKind, right.adapterKind);
};
export const selectProvider = (registry, input) => {
    ...
    const eligibleProviders = providers.filter((provider) => {
        ...
        const capabilityMatch = provider.capabilities.some((capability) => capability.category === request.category) ||
    ...
    if (eligibleProviders.length === 0) {
        return ok(ProviderSelectionResultSchema.parse({
...
//# sourceMappingURL=provider-selection-policy.js.map