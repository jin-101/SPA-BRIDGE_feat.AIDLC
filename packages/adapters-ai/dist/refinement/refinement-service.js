...
import { ProviderInvocationRequestSchema, ProviderNeutralRefinementRequestSchema, ProviderSelectionRequestSchema, RefinementResultSchema, } from '../types.js';
...
import { defaultRetryStrategy, invokeWithTimeout } from '../invocation/provider-timeout-guard.js';
import { validateProviderResponse } from '../validation/provider-response-validator.js';
const buildProviderRegistryOrThrow = (providers) => {
...
const blockedResult = (request, providerId, reasonCode, message) => RefinementResultSchema.parse({
    status: 'blocked',
    suggestions: [],
...
export const refineMapping = async (request, dependencies) => {
    ...
    const invocationResult = await invokeWithTimeout(adapter, invocationRequest, dependencies.config.retryStrategy ?? defaultRetryStrategy);
    if (!invocationResult.ok) {
        const diagnostic = createProviderFailureDiagnostic(invocationResult.error, invocationRequest);
        const audit = buildInvocationAuditEvent(invocationRequest, 'provider-invocation-failed', invocationResult.error.code === 'TIMEOUT' ? 'warning' : 'error', [invocationResult.error.code], invocationResult.error.message, { blockedDecisions: 1 });
        return ok(RefinementResultSchema.parse({
            status: 'partial',
            suggestions: [],
    ...
    const audit = buildInvocationAuditEvent(invocationRequest, 'provider-invocation-succeeded', 'info', ['PROVIDER_SELECTED'], createSafeDisplayString(`Provider ${provider.providerId} returned ${suggestions.length} suggestions`), { suggestions: suggestions.length });
    return ok(RefinementResultSchema.parse({
        status: 'succeeded',
        suggestions,
        diagnostics: [],
