...
const toProviderSuggestionText = (value, fallback) => {
...
const buildResponseFromText = (descriptor, request, text, modelLabel) => {
        ...
        suggestions: [
            {
                ...
                confidence,
                sourceRefs: request.context.safeRefs.filter((ref) => ref.kind === 'source'),
                generatedRefs: request.context.safeRefs.filter((ref) => ref.kind === 'generated'),
                provenance: {
                    providerId: descriptor.providerId,
                    adapterKind: descriptor.adapterKind,
                    modelLabel: createSafeDisplayString(modelLabel),
...
const invokeOllama = async (descriptor, request) => {
...
const invokeOpenAiCompatible = async (descriptor, request) => {
