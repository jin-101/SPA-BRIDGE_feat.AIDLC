...
const escapeHtml = (input) => input
    ...
    .replaceAll('"', '&quot;')
    .replaceAll("'", '&#39;');
const escapeMarkdown = (input) => input.replaceAll('|', '\\|').replaceAll('\n', ' ');
const renderJson = (request) => JSON.stringify({
...
const renderHtml = (request) => {
    const sections = request.payload.sections
        .map((section) => `<section><h2>${escapeHtml(section.title)}</h2>${section.lines.map((line) => `<p>${escapeHtml(line)}</p>`).join('')}</section>`)
        .join('');
...
export const createDefaultReportBridge = () => ({
    async exportReport(request) {
        const content = request.reportFormat === 'markdown'
...
//# sourceMappingURL=report-bridge.js.map