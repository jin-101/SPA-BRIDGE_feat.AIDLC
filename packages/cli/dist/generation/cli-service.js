...
export const createDefaultCliRuntime = (overrides = {}) => ({
...
export const runCli = async (argv, env = process.env, runtimeOverrides = {}) => {
    ...
    const shouldProceed = await confirmationCoordinator.requestConfirmation(resolvedResult.value.executionMode, `Proceed with ${parsedResult.value.commandName}?`, false, parsedResult.value.commandName === 'report');
    if (!shouldProceed) {
