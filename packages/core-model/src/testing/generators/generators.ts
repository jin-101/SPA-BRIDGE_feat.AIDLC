...
import { IntermediateRepresentationSchema, IrComponentSchema } from '../../ir/ir.js';
import { ConversionReportSchema, RunSummarySchema } from '../../report/report.js';
...
} from '../../traceability/traceability.js';
...
const jsonRoundTripValue = fc.jsonValue().map((value) => JSON.parse(JSON.stringify(value)) as unknown);
...
const angularSourceModelRef = fc
  .record({
    ...
    projectKind: fc.constantFrom('application', 'library', 'workspace' as const),
  })
...
const traceLink = fc
  .record({
    ...
    relation: fc.constantFrom('maps-to', 'derived-from', 'emits', 'references' as const),
...
const diagnostic = fc
  .record({
    ...
    severity: fc.constantFrom('info', 'warning', 'error', 'manual-review', 'security-blocker' as const),
    message: safeString,
...
const report = fc
  .record({
    ...
    runSummary,
    convertedFiles: fc.array(
      fc.record({
        sourcePath: fc.string({ minLength: 1, maxLength: 16 }),
        outputPath: fc.string({ minLength: 1, maxLength: 16 }),
        status: fc.constantFrom('generated', 'updated', 'unchanged', 'skipped' as const),
      }),
      { maxLength: 5 },
    ),
    diagnostics: diagnosticsCollection,
    qualityResults: fc.array(
      fc.record({
        gate: fc.constantFrom('typecheck', 'lint', 'format', 'build', 'unit-tests', 'property-tests' as const),
        status: fc.constantFrom('passed', 'failed', 'skipped' as const),
    ...
    manualReviewItems: fc.array(
      fc.record({
        id: fc.string({ minLength: 1, maxLength: 16 }),
        title: safeString,
        description: fc.option(safeString, { nil: undefined }),
        status: fc.constantFrom('open', 'resolved', 'deferred' as const),
      }),
      { maxLength: 5 },
    ),
  })
