...
import { createMigrationRegistry, createRedactedString, createSafeDisplayString, err, isErr, isOk, ok, coreModelArbitraries, createDiagnostic, createMaskToken, buildTraceabilityIndexes, validateDiagnosticsCollection, validateTraceabilityMap, IntermediateRepresentationSchema, MaskTokenMapSchema, AngularSourceModelBoundarySchema, ConversionReportSchema, DiagnosticSchema, } from '../src/index.js';
...
describe('core model schemas', () => {
    test('IR schema round-trips through JSON', () => {
        fc.assert(fc.property(coreModelArbitraries.component, (component) => {
            const input = {
                schemaVersion: 1,
    ...
    test('traceability and diagnostics preserve canonical shape', () => {
        fc.assert(fc.property(coreModelArbitraries.traceabilityMap, coreModelArbitraries.diagnostic, (traceabilityMap, diagnostic) => {
    ...
    test('diagnostics round-trip through JSON without losing severity', () => {
        fc.assert(fc.property(coreModelArbitraries.diagnostic, (diagnostic) => {
    ...
    test('traceability indexes preserve link membership', () => {
        fc.assert(fc.property(coreModelArbitraries.traceabilityMap, (traceabilityMap) => {
    ...
    test('report and manifest schemas round-trip through JSON', () => {
        fc.assert(fc.property(coreModelArbitraries.report, (report) => {
            const parsed = ConversionReportSchema.parse(JSON.parse(JSON.stringify(report)));
            expect(parsed).toEqual(report);
        }));
    });
    test('mask token map schema round-trips', () => {
        fc.assert(fc.property(coreModelArbitraries.maskTokenMap, (maskTokenMap) => {
    ...
    test('Angular source boundary schema round-trips', () => {
        fc.assert(fc.property(coreModelArbitraries.sourceModelBoundary, (boundary) => {
...
describe('migration registry', () => {
    ...
    test('migrates to the latest version', () => {
        const registry = createMigrationRegistry(2);
...
//# sourceMappingURL=core-model.test.js.map