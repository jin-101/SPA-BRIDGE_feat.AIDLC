...
export type GeneratedTargetValidationDiagnosticCategory = 'dependency-install-failure' | 'next-client-boundary-missing' | 'typescript-import-resolution' | 'typescript-helper-missing' | 'typescript-alias-resolution' | 'next-build-config' | 'style-or-asset-reference' | 'lint-or-format' | 'unsafe-command' | 'timeout' | 'manual-review-required';
...
export type GeneratedTargetDeterministicFix = {
    fixerId: string;
    category: 'next-client-boundary' | 'missing-helper-import' | 'package-manifest' | 'dependency-replacement' | 'alias' | 'typescript-config' | 'import-path' | 'style-or-module-reference' | 'filename-or-path';
    summary: string;
