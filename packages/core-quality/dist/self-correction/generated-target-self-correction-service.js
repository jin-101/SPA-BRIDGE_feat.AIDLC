...
import { GeneratedTargetCommandPlanner } from './generated-target-command-planner.js';
export class GeneratedTargetSelfCorrectionService {
    planner;
    fixerRegistry;
    aiBoundary;
    constructor(planner = new GeneratedTargetCommandPlanner(), fixerRegistry = new DeterministicFixerRegistry(), aiBoundary = new AiRepairBoundary()) {
    ...
    evaluate(input) {
        ...
        const validationResults = input.validationResults ?? commandPlan.commands.map((command) => ({
            ...
            kind: command.kind,
            status: command.allowlisted ? 'planned' : 'blocked',
            durationMs: 0,
        ...
        const remainingBlockerIds = diagnostics
            ...
            .sort();
        const aiRepairRequests = remainingBlockerIds.length > fixes.length
        ...
        const status = failedCommands === 0 && remainingBlockerIds.length === 0
            ? passedCommands > 0
                ? 'passed'
                : 'skipped'
...
//# sourceMappingURL=generated-target-self-correction-service.js.map