...
const countMatches = (files, pattern) => files.reduce((total, file) => total + (file.content.match(pattern)?.length ?? 0), 0);
const looksEmptyComponent = (file) => {
...
const hasScript = (packageJson, scriptName) => {
    const scripts = packageJson?.scripts;
    return Boolean(scripts && typeof scripts === 'object' && scriptName in scripts);
};
const hasAllPaths = (paths, expected) => expected.every((path) => paths.has(path));
const parseAnimationSummary = (files) => {
...
export class RuntimeParityQualityGate {
    evaluate(input) {
        ...
        const expectedFiles = input.expectedFramework === 'nextjs'
            ? nextRequiredFiles
        ...
        const selfCorrectionPenalty = selfCorrection?.status === 'blocked'
            ? 20
            : selfCorrection?.status === 'degraded'
                ? 10
                : 0;
        const penalties = missingRequiredFiles.length * 15
...
//# sourceMappingURL=runtime-parity-quality-gate.js.map