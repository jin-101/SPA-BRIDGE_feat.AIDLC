import type {
  GeneratedTargetDeterministicFix,
  GeneratedTargetValidationDiagnostic,
} from '../types.js';

export class DeterministicFixerRegistry {
  planFixes(diagnostics: GeneratedTargetValidationDiagnostic[]): GeneratedTargetDeterministicFix[] {
    const fixes = diagnostics
      .flatMap((diagnostic) => diagnostic.fixerCandidateIds.map((fixerId) => this.createFix(fixerId, diagnostic)))
      .filter((fix): fix is GeneratedTargetDeterministicFix => Boolean(fix));

    return [...new Map(fixes.map((fix) => [fix.idempotenceKey, fix])).values()].sort((left, right) =>
      left.fixerId.localeCompare(right.fixerId),
    );
  }

  private createFix(fixerId: string, diagnostic: GeneratedTargetValidationDiagnostic): GeneratedTargetDeterministicFix | undefined {
    const base = {
      fixerId,
