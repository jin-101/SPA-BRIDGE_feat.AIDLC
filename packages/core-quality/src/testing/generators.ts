...
export const qualityRequestArbitrary: fc.Arbitrary<QualityRequest> = fc.record({
...
export const qualityGateDefinitionArbitrary: fc.Arbitrary<QualityGateDefinition> = fc.record({
  gateId: nonEmptyString,
  displayName: nonEmptyString,
  order: fc.integer({ min: 1, max: 1000 }),
  kind: fc.constantFrom<QualityGateKind>('build', 'lint', 'format', 'unit', 'integration', 'property'),
  blocking: fc.boolean(),
...
export const propertyTestPlanArbitrary: fc.Arbitrary<PropertyTestPlan> = fc.record({
...
export const qualityToolKindArbitrary: fc.Arbitrary<QualityToolKind> =
  fc.constantFrom('build', 'lint', 'format', 'unit', 'integration', 'property');
