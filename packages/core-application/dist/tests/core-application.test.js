...
import { ConfigPipeline, ConversionApplicationService, FixedClock, FixedRandomness, InMemoryAudit, InMemoryFileSystem, InMemoryLogger, InMemoryReportExporter, ManifestStateMachine, PathGuard, PolicyGate, ResumePlanner, RunStatusReader, RunWorkspaceManager, } from '../src/index.js';
...
describe('RunStatusReader and ResumePlanner', () => {
    it('reads manifest status and builds a resume plan from the last checkpoint', async () => {
        ...
        const manifest = manifestMachine.finalizeFailed(manifestMachine.recordCheckpoint(manifestMachine.createRunningManifest({
            runId: 'run-1',
            projectRoot: '/repo',
            inputPath: '/repo/app',
            outputPath: '/repo/out',
            startedAt: '2026-06-04T00:00:00.000Z',
        }), { id: 'checkpoint-1', stepId: 'scan', artifactRefs: [], completedAt: '2026-06-04T00:01:00.000Z' }).value, 'boom');
        ...
        const steps = [
            {
                ...
                execute: async () => ({
                    ...
                    value: {
                        ...
                        manualReviewItems: [{ id: 'review-1', title: createSafeDisplayString('Check mapping'), status: 'open' }],
                        nextStatus: 'running',
                    },
                }),
            },
...
//# sourceMappingURL=core-application.test.js.map