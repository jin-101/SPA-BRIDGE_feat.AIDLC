...
- [x] Validate PBT property candidates for UOW-02.
...
- **Shared Service Scope**: Own run start, config validation, workspace initialization, manifest persistence, and orchestration for CLI/Web UI.
- **Configuration Precedence**: Project config file first, with CLI/UI overrides taking precedence.
- **Run Workspace**: Hidden workspace-root directory, `.spa-bridge/runs/<runId>`.
- **Manifest Lifecycle**: Created at run start, updated during execution, finalized at completion.
- **Adapter Boundary**: Core application coordinates config, workspace, policy, and report handoff; parsing/transformation remain in downstream units.
- **Error Strategy**: Fail-fast while preserving partial artifacts and diagnostic state.
- **Provider Policy**: Core application enforces policy; providers are only called through ports/policy.
- **Report Flow**: Core application always creates canonical report state and delegates markdown/html exports through exporter ports.
- **Resume Behavior**: Recoverable runs can resume from manifest state.
- **Manual Review State**: Stored in manifest/report so CLI and Web UI can display it.
